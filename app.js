// ===== DEMO MODE =====
const DEMO_LIMIT = 3;
const UNLOCK_CODE = 'CALMPAWS2026';
const STORAGE_UNLOCKED = 'calmpaws_unlocked';
let isUnlocked = localStorage.getItem(STORAGE_UNLOCKED) === 'true';

function checkDemoLimit() {
  if (isUnlocked) return true;
  if (db.logs.length >= DEMO_LIMIT) {
...
function updateDemoBadge() {
  if (isUnlocked) { document.getElementById('demo-badge').classList.add('hidden'); return; }
  const remaining = Math.max(0, DEMO_LIMIT - db.logs.length);
  const badge = document.getElementById('demo-badge');
  badge.classList.remove('hidden');
  badge.textContent = remaining > 0 ? `🔓 Demo: ${remaining} log${remaining !== 1 ? 's' : ''} left` : '🔒 Demo limit reached';
}
...
let selectedReactivity = null;
let selectedBaseReactivity = null;
let logProgress = null;
let pendingDeleteId = null;
let reactivityChart = null;
let distanceChart = null;

const TIPS = [
  "Keep sessions short (5–10 min). Reactive dogs fatigue fast. Quality over quantity!",
  "Always end on success — even if it's tiny. A calm sniff counts.",
  "Threshold distance is your best friend. Staying 'under threshold' = real learning happens.",
  "BAT tip: Let your dog make the decision to disengage from the trigger. Don't rush it.",
  "Trigger stacking is real — a tired + hungry dog has a much lower threshold. Track their day.",
  "The 'Look at That' game: dog looks at trigger → click → treat. You're building a positive association.",
  "Scatter feeding (tossing treats on the ground) activates the sniffing brain, which calms the nervous system.",
  "Celebrate tiny wins. 10 ft closer than last week? That's HUGE progress worth celebrating.",
  "If your dog reacted: no punishment. Just calmly exit. The reaction already happened — move forward.",
  "Distance is the biggest tool you have. Always ask: can I add 5 more feet?"
];
...
// ===== STORAGE =====
function loadData() {
  try { db = JSON.parse(localStorage.getItem('calmpaws_db')) || db; } catch(e) {}
}
...
function saveProfile() {
  db.profile = {
    ...
    trainer: document.getElementById('trainer-name').value.trim(),
    method: document.getElementById('training-method').value,
    vetNotes: document.getElementById('vet-notes').value.trim(),
    photo: db.profile?.photo || null
  };
...
function renderProfile() {
  ...
  const p = db.profile;
  setValue('dog-name', p.name);
  setValue('dog-breed', p.breed);
  setValue('dog-age', p.age);
  setValue('trainer-name', p.trainer);
  setSelectValue('training-method', p.method);
  setValue('vet-notes', p.vetNotes);
  if (p.photo) {
    const prev = document.getElementById('photo-preview');
    prev.innerHTML = `<img src="${p.photo}" alt="Dog photo">`;
  }
  updateHeader();
}
...
function updateHeader() {
  ...
  if (db.profile?.photo) {
    document.getElementById('dog-avatar-header').innerHTML = `<img src="${db.profile.photo}" alt="dog">`;
  }
}
...
function addTrigger() {
  ...
  if (!name) return;
  if (db.triggers.find(t => t.name.toLowerCase() === name.toLowerCase())) {
    showToast('Trigger already exists'); return;
  }
  ...
  renderTriggerTags();
  renderTriggerSelect();
  showToast('Trigger added! ⚠️');
}

function removeTrigger(id) {
  db.triggers = db.triggers.filter(t => t.id !== id);
  saveData(); renderTriggerTags(); renderTriggerSelect();
}
...
function renderTriggerTags() {
  ...
  el.innerHTML = db.triggers.map(t => `
    <span class="trigger-tag ${t.severity}">
      ${t.name}
      <button onclick="removeTrigger(${t.id})" title="Remove">×</button>
    </span>`).join('');
}
...
function renderTriggerSelect() {
  ...
  sel.innerHTML = '<option value="">Select a known trigger…</option>' +
    db.triggers.map(t => `<option value="${t.name}">${t.name}</option>`).join('');
...
function saveLog(e) {
  ...
  const log = {
    ...
    trigger,
    distance: parseInt(document.getElementById('log-distance').value),
    reactivity: selectedReactivity,
    technique: document.getElementById('log-technique').value,
...
function resetLogForm() {
  ...
  document.getElementById('log-distance').value = 20;
  document.getElementById('distance-val').textContent = '20 ft';
  document.querySelectorAll('#reactivity-btns .r-btn').forEach(b => b.classList.remove('active'));
  document.getElementById('log-notes').value = '';
  document.getElementById('reactivity-desc').textContent = 'Tap a number to rate the reaction';
  selectedReactivity = null; logProgress = null;
  document.getElementById('prog-yes').className = 'prog-btn';
  document.getElementById('prog-no').className = 'prog-btn';
}
  ...
  el.innerHTML = db.logs.slice(0, 20).map(log => {
    ...
    return `<div class="log-item">
      <div class="log-item-header">
        <span class="log-item-date">📅 ${dateStr} · ${timeStr}</span>
        <div class="log-item-react">
          <span class="react-badge ${rClass}">${log.reactivity}</span>
        </div>
      </div>
      <div class="log-item-body">
        <span class="log-trigger-tag">⚠️ ${log.trigger}</span>
        <div class="log-meta">
          <span>📏 ${log.distance} ft</span>
          <span>🎓 ${log.technique}</span>
        </div>
        ${log.notes ? `<div class="log-notes-text">"${log.notes}"</div>` : ''}
      </div>
      <div class="log-item-footer">
        <span class="progress-chip ${log.progress ? 'yes' : 'no'}">${log.progress ? '✅ Progress!' : '🔄 Keep going'}</span>
        <button class="delete-btn" onclick="confirmDelete(${log.id})">🗑 Delete</button>
      </div>
    </div>`;
  }).join('');
}
...
function updateStats() {
  ...
  const avgR = avg(db.logs.map(l => l.reactivity)).toFixed(1);
  const avgD = Math.round(avg(db.logs.map(l => l.distance)));
  const successRate = Math.round((db.logs.filter(l => l.progress).length / db.logs.length) * 100);
  document.getElementById('stat-avg-reactivity').textContent = avgR;
  document.getElementById('stat-avg-distance').textContent = avgD;
...
function renderCharts() {
  ...
  sorted.forEach(log => {
    ...
    weekData[week].reactivity.push(log.reactivity);
    weekData[week].distance.push(log.distance);
  });
  ...
  const chartDefaults = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { backgroundColor: '#1e1c30', borderColor: '#2e2b4a', borderWidth: 1, titleColor: '#f1f0ff', bodyColor: '#a09ec0' } }, scales: { x: { grid: { color: '#2e2b4a' }, ticks: { color: '#6b698a', font: { size: 11 } } }, y: { grid: { color: '#2e2b4a' }, ticks: { color: '#6b698a', font: { size: 11 } } } } };

  if (reactivityChart) reactivityChart.destroy();
  reactivityChart = new Chart(document.getElementById('reactivityChart'), {
    type: 'line',
    data: { labels, datasets: [{ data: rData, borderColor: '#7c6ef7', backgroundColor: 'rgba(124,110,247,.15)', fill: true, tension: .4, pointBackgroundColor: '#a78bfa', pointRadius: 5 }] },
  ...
  distanceChart = new Chart(document.getElementById('distanceChart'), {
    type: 'bar',
    data: { labels, datasets: [{ data: dData, backgroundColor: 'rgba(52,211,153,.3)', borderColor: '#34d399', borderWidth: 2, borderRadius: 6 }] },
  ...
  document.getElementById('trigger-bars').innerHTML = sorted2.map(([name, count]) =>
    `<div class="t-bar-row">
      <div class="t-bar-label">${name}</div>
      <div class="t-bar-track"><div class="t-bar-fill" style="width:${(count/max*100).toFixed(0)}%"></div></div>
      <div class="t-bar-count">${count}</div>
    </div>`).join('') || '<p style="color:var(--text3);font-size:.85rem">No data yet</p>';
}
...
function renderHeatmap() {
  ...
  const logDates = {};
  db.logs.forEach(l => {
    const d = new Date(l.datetime).toDateString();
...
function updateStreak() {
  ...
  let d = new Date();
  while (true) {
    if (logDates.has(d.toDateString())) { streak++; d.setDate(d.getDate() - 1); }
    else break;
  }
  if (streak >= 2) {
    document.getElementById('streak-text').textContent = `🔥 ${streak}-day tracking streak! Keep it up!`;
    banner.classList.remove('hidden');
    document.querySelector('.main-content').classList.add('with-streak');
  } else {
    banner.classList.add('hidden');
    document.querySelector('.main-content').classList.remove('with-streak');
  }
}
...
function renderReminder() {
  ...
  const d = new Date(db.reminder.dt);
...
function renderReport() {
  ...
  if (db.logs.length) {
    ...
    const rows = db.logs.slice(0, 30).map(l => {
      const d = new Date(l.datetime);
      return `<tr><td>${d.toLocaleDateString()}</td><td>${l.trigger}</td><td>${l.distance} ft</td><td>${l.reactivity}/10</td><td>${l.technique}</td><td>${l.progress ? '✅' : '🔄'}</td></tr>`;
    }).join('');
    ...
    const trigCounts = {};
    db.logs.forEach(l => { trigCounts[l.trigger] = (trigCounts[l.trigger] || 0) + 1; });
    document.getElementById('report-triggers').innerHTML = Object.entries(trigCounts).sort((a,b)=>b[1]-a[1]).map(([n,c]) => `<span class="report-trigger-chip">⚠️ ${n} (${c}×)</span>`).join('');
  }
}
...
printStyle.textContent = `@media print{.bottom-nav,.header,.streak-banner,.report-actions,#toast,.modal-overlay{display:none!important}.report-preview{background:#fff;color:#000;border:none;padding:0}.report-table td,.report-table th{color:#000}}`;
